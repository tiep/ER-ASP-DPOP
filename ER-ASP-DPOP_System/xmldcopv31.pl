...
processing(X, Out):-
	...
	member(element(beliefjoints,_,BeliefJoints),X),
	member(element(probUtilities,_,ProbUtils),X),
	...
	process_beliefjoint(BeliefJoints,ListBJs),
        process_beliefjointprob(ProbUtils,ListProbUtils),
	define_agents(ListA),
	define_variables(ListV, ListD),
	define_constraints(ListC),
	print('relation-----'),
	define_relations(ListR),
	define_jointbeliefs(ListBJs),
	define_probUtils(ListProbUtils),
	print_problem(Out),
	print_list_agent.
...
print_problem_agents(Out, [Agent|LA]):-
	...
	print_jointbeliefprobs,
	print_constraints(LCons),
	print_bad_constraints(LCons2),
 	findall((CN,JBN,P,Vs), (utilProb(CN,JBN,P,Vs)), UtilProbs),
        print_util_prob(UtilProbs),
...
print_related_constraints([(N,_,S)|CDesc]):-
	...
	format('constraint_prob(c~w,c~w_p).\n',[N,N]),
	format('constraint_member(c~w,rd_~w).\n',[N,N]),
...
print_jointbelief:-
	findall(X,jb(X),ListJBs),
	length(ListJBs,N),
	format('jbn(~q).\n',[N]),
	print_jointbelief(ListJBs).

print_jointbelief([]).
print_jointbelief([H|T]):-
	format('jb(~w).\n',[H]),
	print_jointbelief(T).

print_jointbeliefprobs:-
	findall(jbp(X,Y),jbp(X,Y),JBPs),
	print_jointbeliefprob(JBPs).
print_jointbeliefprob([]).
print_jointbeliefprob([H|T]):-
	format('~w.\n',[H]),
	print_jointbeliefprob(T).
...
print_constraints([(N,Scope, Util, Values)|LCons]):-
	...
	append([randomvariable],Scope,ScopeNew),
	print_value_pairs(ScopeNew, Values),
	format(').\n',[]),
	%%length(Scope,Length),
	%%check constraint for either min value or max value	
	%%(Length == 1 -> 
...
define_probUtils([]).
define_probUtils([(C,JB, [pcdata(Values)])|LR]):-
        %%% format('\nRelations ****** next ~q \n ~q\n',[Values, LR]),
        atom_codes(CN, C),
        atom_codes(JBN,JB),
        %trace,
        (atom(Values) ->
            define_probUtil(CN,JBN, Values);
            (
             atom_codes(RValues, Values),
             define_probUtil(CN,JBN, RValues)
            )
        ),
        %print('\n\n finish doing ----------------: \n'),
        %notrace,
...
define_probUtil(CN,JBN, Values):-
        testing_prob(CN,JBN,Values),
        print('DONEDONE\n').

testing_prob(R,JBN,Values):-
        atom_chars(Values,ListValues),
        process_one_by_one_prob(R,JBN,ListValues),
        print('DDDDDDDDDDDD\n').


process_one_by_one_prob(R,JBN,LV):-
        member('|',LV),
        append(C1,['|'|C2],LV),
        \+ member('|',C1),
        append(CVValues,[':'|CScope],C1),
        ...
        atomic_list_concat(LLVars, ' ', Scope),
        subtract(LLVars, [''], LVars),
        assert(utilProb(R, JBN,VValues, LVars)),

        process_one_by_one_prob(R,JBN,C2,VValues).
...
process_one_by_one_prob(R,JBN,LV,Value):-
        member('|',LV),
        append(C1,['|'|C2],LV),
        \+ member('|',C1), %to be sure append clause retrieves the first element
    %trace,
...
process_one_by_one_prob(R,JBN,C2,Value):-
        \+member('|',C2),
        (append(CVValues,[':'|CScope],C2) ->
                atom_chars(VValues,CVValues),
                atom_chars(Scope,CScope),
...
define_jointbeliefs([]).
define_jointbeliefs([(JBN,PJB)|ListA]):-
         atom_codes(JBNC,JBN),
	 atom_codes(PJBC,PJB),
         assert(jb(JBNC)),
	 assert(jbp(JBNC,PJBC)),
         define_jointbeliefs(ListA).
...
process_beliefjoint(BeliefJoints,ListBJs):-
	findall((N,P),
	  (
		member(element(beliefjoint,NP,_), BeliefJoints),
		member(name=N,NP),
		member(prob=P,NP)
	  ),
	  ListBJs).
 ...
 process_beliefjointprob(ProbUtils,ListProbUtils):-
	findall((CN,JBN,T),
		(member(element(probutil,NDTA,T),ProbUtils),
		 member(consname=CN,NDTA),
		 member(jbname=JBN,NDTA)
		), ListProbUtils).
