...
process_total_time:-
	...
	findall(TR, XR^delta_downR(XR,TR), LTR),
        max_member(MaxR, LTR),
        delta_upR(Root,TimeUpR),
	NewRunning is TimeUp + Max + TimeUpR + MaxR,
...
calculate_timeR(Max):-
        ...
        numberAgent(NumAgent),
        %format('-----hehehe ~w , numagent ~w.\n',[C,NumAgent]),
        (C > Max -> true ; calculate_time1R(C),NewC is C + 1, assert(countingR(NewC)), calculate_timeR(Max)).

calculate_time1R(C):-
        findall(A, rank(A,C), L),
        (C==1 -> L=[Node], assert(delta_downR(Node,0)) ;
        delta_downR(L)).
...
delta_downR([H|T]):-
        ...
        member(parent(H,P),Topo),
        delta_downR(P,TimeP),
        runningtime_downR(H,TimeH),
        NewTime is TimeP + TimeH,
        assert(delta_downR(H,NewTime)),
        %format('delta_down(~w,~w).\n',[H,NewTime]),
        delta_downR(T).
...
process_nextR([H|T]):-
        ...
        (subset(LC1,Done) ->
                ...
                assert(doneR(NewLC1)),
                %format('------LC1 is ~w->>>>>>\n',[LC1]),
                findall(Ti, (member(A,LC1), delta_upR(A,Ti)), LT),
                %format('------done is ~w->>>>>>\n',[LT]),
                max_member(Max,LT),
                runningtime_upR(H,Time),
                NewTime is Time + Max,
                %format('delta_up(~w,~w).\n',[H,NewTime])
                assert(delta_upR(H,NewTime)) ; true),
        process_nextR(T).
...
get_upR:-                                                                                  
        in(to(master,runningtime_upR(Name,UpR))),                                           
        assert(runningtime_upR(Name,UpR)),                                                  
        %format('rank(~w,~w,~w).\n', [Name,Rank,Time]),                                   
        findall((N-U), runningtime_upR(N,U), L),                                           
        length(L,Le),                                                                     
        %format('numbernow(~w).\n',[Le]),                                                 
        %format('<<<<<<<<<<runningtime_up(~w,~w).\n',[Name,Up]),                          
        numberAgent(NumAgent),                                                            
        (Le == NumAgent-> true ; get_upR                                                   
        ).                                                                                
                                                                                          
get_downR:-                                                                                
        in(to(master,runningtime_downR(Name,DownR))),                                       
        assert(runningtime_downR(Name,DownR)),                                              
        %format('rank(~w,~w,~w).\n', [Name,Rank,Time]),                                   
        findall((N-D), runningtime_downR(N,D), L),                                         
        length(L,Le),                                                                     
        %format('numbernow(~w).\n',[Le]),                                                 
        %format('<<<<<<<<<<<runningtime_down(~w,~w).\n',[Name,Down]),                     
        numberAgent(NumAgent),                                                            
        Num is NumAgent - 1,                                                              
        (Le == Num-> true ; get_downR                                                      
        ).
